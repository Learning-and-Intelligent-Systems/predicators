...
class StickyTableEnv(BaseEnv):
    ...
    cube_scale: ClassVar[float] = 0.1  # as a function of table radius
    ...
    def __init__(self, use_gui: bool = True) -> None:
        ...
        self._table_type = Type("target", ["x", "y", "radius", "is_sticky"])
    ...
    def types(self) -> Set[Type]:
        return {self._cube_type, self._target_type}
    ...
    def render_state_plt(
        ...
        for table in tables:
            ...
            if is_sticky:
                wedge = Wedge((x, y), radius, 0, 180, fill=False, hatch="/")
                ax.add_artist(wedge)
        x = state.get(cube, "x")
        y = state.get(cube, "y")
        size = state.get(cube, "size")
        rect = utils.Rectangle(x, y, size, size, 0.0)
    ...
    def _get_tasks(self, num: int,
        ...
        while len(tasks) < num:
            ...
            thetas = np.linspace(0, 2 * np.pi, num=num_tables)
            angle_diff = thetas[1] - thetas[0]
            radius = d * np.sin(angle_diff / 2)
            # Select the radius to prevent any overlap. Exact would be
            # 2 * d * sin(theta / 2). Divide by 2 to be conservative.
            ...
            x_lb = state_dict[init_table]["x"] - radius + size
            x_ub = state_dict[init_table]["x"] + radius - size
            y_lb = state_dict[init_table]["y"] - radius + size
            y_ub = state_dict[init_table]["y"] + radius - size
            ...
            assert self._OnTable_holds(state, [cube, init_table])
    ...
    def _OnTable_holds(self, state: State, objects: Sequence[Object]) -> bool:
        cube, table = objects
        size = state.get(table, "size")
        radius = state.get(table, "radius")
        x_lb = state.get(table, "x") - radius + size
        x_ub = state.get(table, "x") + radius - size
        y_lb = state.get(table, "y") - radius + size
        y_ub = state.get(table, "y") + radius - size
        return x_lb < state.get(cube, "x") < x_ub and \
               y_lb < state.get(cube, "y") < y_ub
    ...
    def _HandEmpty_holds(self, state: State,
        ...
        assert not objects
        cube, = state.get(self._cube_type)
