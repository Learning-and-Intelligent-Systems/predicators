...
class PyBulletShelfEnv(PyBulletEnv):
    ...
    # Robot parameters.
    _robot_init_x: ClassVar[float] = (_x_lb + _x_ub) / 2
    ...
    def _initialize_pybullet(self) -> None:
        ...
        # Create shelf.
        color = self._shelf_color
        orientation = self._default_orn
        base_pose = (self._shelf_x, self._shelf_y, self._table_height + self._shelf_base_height / 2)
        # Shelf base.
        # Create the collision shape.
        base_half_extents = [
            self._shelf_width / 2, self._shelf_length / 2,
            self._shelf_base_height / 2
        ]
        ...
        pose = (
            0, 0,
            self._shelf_base_height / 2 + self._shelf_ceiling_height - \
                self._shelf_ceiling_thickness / 2
        )
        ...
        for x_sign in [-1, 1]:
            for y_sign in [-1, 1]:
                pose = (x_sign * (self._shelf_width - self._shelf_pole_diam) /
                        2, y_sign *
                        (self._shelf_length - self._shelf_pole_diam) / 2,
                        self._shelf_base_height / 2 +
                        self._shelf_ceiling_height / 2)
                link_positions.append(pose)
                half_extents = [
                    self._shelf_pole_diam / 2, self._shelf_pole_diam / 2,
                    self._shelf_ceiling_height / 2
                ]
                collision_id = p.createCollisionShape(
                    p.GEOM_BOX,
        ...
        color = self._shelf_color
        orientation = self._default_orn
        pose = (self._wall_x, (self._y_lb + self._y_ub) / 2,
                self._table_height + self._wall_height / 2)
        # Create the collision shape.
        half_extents = [
            self._wall_thickness / 2, (self._y_ub - self._y_lb) / 2,
            self._wall_height / 2
        ]
        collision_id = p.createCollisionShape(
            p.GEOM_BOX,
    ...
    def _create_pybullet_robot(
        ...
        ee_home = Pose(
            (self._robot_init_x, self._robot_init_y, self._robot_init_z),
    ...
    def _create_move_to_shelf_place_option(self):
        ...
        tx = self._shelf_x
        ty = self._shelf_y
        tz = self._table_height + self._shelf_base_height + self._block_size / 2 + self._offset_z
        ...
        return create_move_end_effector_to_pose_option(
            ...
            get_collision_bodies=lambda _1, _2: {self._shelf_id_sim},
