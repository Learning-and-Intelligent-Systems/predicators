...
class BurgerNoMoveGroundTruthNSRTFactory(GroundTruthOptionFactory):
    ...
    def _create_slice_policy(cls) -> ParameterizedPolicy:
        ...
        def policy(state: State, memory: Dict, objects: Sequence[Object],
            ...
            move_action = self._move_subpolicy(state, robot, item)
    ...
    def _create_pickplace_policy(cls) -> ParameterizedPolicy:
        ...
        def policy(state: State, memory: Dict, objects: Sequence[Object],
                   params: Array) -> Action:
            del memory, params  # unused
            # Pick has two object arguments, Place has three.
            robot, item = objects[:2]
            move_action = self._move_subpolicy(state, robot, item)
            pickplace_action = Action(np.array([0, 0, -1, 0, 1], dtype=np.float32))
            if move_action is not None:
                return move_action
            return pickplace_action
