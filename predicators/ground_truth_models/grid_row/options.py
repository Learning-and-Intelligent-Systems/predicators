...
class GridDoorGroundTruthOptionFactory(GridRowDoorGroundTruthOptionFactory):
    """Ground-truth options for the grid door environment."""
    ...
    def get_options(cls, env_name: str, types: Dict[str, Type],
        ...
        def _MoveTo_policy(state: State, memory: Dict,
            ...
            rob_x = state.get(robot, "x")
            target_x = state.get(target_cell, "x")
            rob_y = state.get(robot, "y")
            target_y = state.get(target_cell, "y")
            dx = target_x - rob_x
            dy = target_y - rob_y
            return Action(np.array([dx, dy, 0.0, 0.0, 0.0], dtype=np.float32))
        ...
        def _toggle_light_policy(state: State, memory: Dict,
            ...
            del state, objects, memory  # unused
            dlight, = params
            return Action(np.array([0.0, 0.0, dlight, 0.0, 0.0], dtype=np.float32))
        ...
        def _move_key_policy(state: State, memory: Dict,
            ...
            del state, objects, memory  # unused
            dmove, = params
            return Action(np.array([0.0, 0.0, 0.0, dmove, 0.0], dtype=np.float32))
        ...
        def _turn_key_policy(state: State, memory: Dict,
            ...
            del state, objects, memory  # unused
            dturn, = params
            return Action(np.array([0.0, 0.0, 0.0, 0.0, dturn], dtype=np.float32))
