...
class CoffeeGroundTruthOptionFactory(GroundTruthOptionFactory):
    ...
    def get_options(env_name: str, types: Dict[str, Type],
        ...
        # Hyperparameters
        twist_policy_tol = 1e-1
        pick_policy_tol = 1e-1
        pour_policy_tol = 1e-1
        ...
        def _get_jug_handle_grasp(state: State,
                                  jug: Object) -> Tuple[float, float, float]:
            # Orient pointing down.
            rot = state.get(jug, "rot") - np.pi / 2
            target_x = state.get(
                jug, "x") + np.cos(rot) * CoffeeEnv.jug_handle_offset
            target_y = state.get(
                jug, "y") + np.sin(rot) * CoffeeEnv.jug_handle_offset
            target_z = CoffeeEnv.jug_handle_height
            return (target_x, target_y, target_z)

        def _get_jug_z(state: State, jug: Object) -> float:
        ...
        # MoveToTwistJug
        def _MoveToTwistJug_policy(state: State, memory: Dict,
            ...
            # target.
            if xy_sq_dist < twist_policy_tol:
                return _get_move_action(jug_top, robot_pos)
            # Move to the position above the jug.
            return _get_move_action((jug_x, jug_y, CoffeeEnv.robot_init_z),
                                    robot_pos)
        ...
        MoveToTwistJug = ParameterizedOption(
            ...
            policy=_MoveToTwistJug_policy,
            initiable=lambda s, m, o, p: True,
            terminal=_MoveToTwistJug_terminal,
        )

        # TwistJug
        def _TwistJug_policy(state: State, memory: Dict,
                             objects: Sequence[Object],
                             params: Array) -> Action:
            ...
            delta_rot = np.clip(desired_rot - current_rot,
                                -CoffeeEnv.max_angular_vel,
                                CoffeeEnv.max_angular_vel)
            if abs(delta_rot) < twist_policy_tol:
                # Move up to stop twisting.
                x = state.get(robot, "x")
                y = state.get(robot, "y")
                z = state.get(robot, "z")
                robot_pos = (x, y, z)
                return _get_move_action((x, y, CoffeeEnv.robot_init_z),
                                        robot_pos)
        ...
        TwistJug = ParameterizedOption(
            ...
            policy=_TwistJug_policy,
            initiable=lambda s, m, o, p: True,
            terminal=_TwistJug_terminal,
        )

        # PickJug
        def _PickJug_policy(state: State, memory: Dict,
                            objects: Sequence[Object],
                            params: Array) -> Action:
            ...
            if target_y > y and xz_handle_sq_dist < pick_policy_tol:
                return _get_move_action(handle_pos, robot_pos)
            # If close enough to the penultimate waypoint in the x/y plane,
            # move to the waypoint (in the z direction).
            if xy_waypoint_sq_dist < pick_policy_tol:
                return _get_move_action((target_x, waypoint_y, target_z),
                                        robot_pos)
            ...
            if safe_z_sq_dist < CoffeeEnv.safe_z_tol:
                return _get_move_action(
                    (target_x, waypoint_y, CoffeeEnv.robot_init_z), robot_pos)
            # Move up to a safe height.
            return _get_move_action((x, y, CoffeeEnv.robot_init_z), robot_pos)
        ...
        PickJug = ParameterizedOption(
            ...
            types=[robot_type, jug_type],
            params_space=Box(0, 1, (0, )),
            policy=_PickJug_policy,
            initiable=lambda s, m, o, p: True,
            terminal=_PickJug_terminal,
        )

        # PlaceJugInMachine
        def _PlaceJugInMachine_policy(state: State, memory: Dict,
                                      objects: Sequence[Object],
                                      params: Array) -> Action:
            ...
            if z > CoffeeEnv.z_lb:
                return _get_move_action(place_pos, jug_pos)
            # Move up.
            return _get_move_action((x, y, z + CoffeeEnv.max_position_vel),
                                    jug_pos)
        ...
        PlaceJugInMachine = ParameterizedOption(
            ...
            params_space=Box(0, 1, (0, )),
            policy=_PlaceJugInMachine_policy,
            initiable=lambda s, m, o, p: True,
            terminal=_PlaceJugInMachine_terminal,
        )

        # TurnMachineOn
        def _TurnMachineOn_policy(state: State, memory: Dict,
                                  objects: Sequence[Object],
                                  params: Array) -> Action:
            ...
            if (CoffeeEnv.button_z - z)**2 < CoffeeEnv.button_radius**2:
                ...
                return _get_move_action(button_pos, robot_pos)
            # Move only in the z direction.
            return _get_move_action((x, y, CoffeeEnv.button_z), robot_pos)
        ...
        TurnMachineOn = ParameterizedOption(
            ...
            params_space=Box(0, 1, (0, )),
            policy=_TurnMachineOn_policy,
            initiable=lambda s, m, o, p: True,
            terminal=_TurnMachineOn_terminal,
        )

        # Pour
        def _Pour_policy(state: State, memory: Dict, objects: Sequence[Object],
                         params: Array) -> Action:
            ...
            jug_x = state.get(jug, "x")
            jug_y = state.get(jug, "y")
            jug_z = _get_jug_z(state, jug)
            jug_pos = (jug_x, jug_y, jug_z)
            pour_x, pour_y, _ = pour_pos = _get_pour_position(state, cup)
            ...
            if sq_dist_to_pour < pour_policy_tol:
                ...
                return _get_move_action(jug_pos, jug_pos, dtilt=dtilt)
            ...
            if xy_pour_sq_dist < CoffeeEnv.safe_z_tol:
                return _get_move_action(pour_pos, jug_pos, dtilt=dtilt)
            # If we're at a safe height, move toward above the pour position.
            if (robot_z - CoffeeEnv.robot_init_z)**2 < CoffeeEnv.safe_z_tol:
                return _get_move_action((pour_x, pour_y, jug_z),
                                        jug_pos,
                                        dtilt=dtilt)
            # Move to a safe moving height.
            return _get_move_action((robot_x, robot_y, CoffeeEnv.robot_init_z),
                                    robot_pos,
                                    dtilt=dtilt)
