...
class OracleClusteringSTRIPSLearner(BaseSTRIPSLearner):
    ...
    that gets its clusters from oracle operators."""

    def _learn(self) -> List[PNAD]:
        # We can only do this learning if data was collected
        # with gt operator annotations.
        assert CFG.offline_data_method == "demo+gt_operators"        
        assert self._dataset.has_annotations and len(self._dataset.annotations) == len(self._dataset.trajectories)
