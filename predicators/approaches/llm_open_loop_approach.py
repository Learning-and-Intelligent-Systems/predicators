...
from predicators.structs import NSRT, Box, Dataset, GroundAtom, Object, \
    ParameterizedOption, Predicate, State, Task, Type, _GroundNSRT, _Option

# TODO: make a new llm_utils.py file for these functions.


def get_llm_based_plan(
    state: State,
    ...
    objects = set(state)
    for option_plan in _get_llm_based_option_plans(atoms, objects, goal,
                                                   options):
        ground_nsrt_plan = _option_plan_to_nsrt_plan(option_plan, atoms,
                                                     objects, goal, nsrts,
                                                     predicates)
...
def _get_llm_based_option_plans(
    ...
    new_prompt = _create_prompt(atoms, goal, [])
    prompt = prompt_prefix + new_prompt
    # Query the LLM.
    llm_predictions = llm.sample_completions(
        prompt=prompt,
        temperature=CFG.llm_temperature,
        seed=CFG.seed,
        num_completions=CFG.llm_num_completions)
    for pred in llm_predictions:
        option_plan = _llm_prediction_to_option_plan(pred, objects, options)
        yield option_plan


def _option_plan_to_nsrt_plan(
        option_plan: List[Tuple[ParameterizedOption,
                                Sequence[Object]]], atoms: Set[GroundAtom],
        objects: Set[Object], goal: Set[GroundAtom], nsrts: Set[NSRT],
        predicates: Set[Predicate]) -> Optional[List[_GroundNSRT]]:
...
def _llm_prediction_to_option_plan(
    ...
    """Convert the output of the LLM into a sequence of ParameterizedOptions
    coupled with a list of objects that will be used to ground the
    ParameterizedOption."""
...
def data_to_prompt_prefix(dataset: Dataset, segmented_trajs,
                          train_tasks: List[Task]) -> str:
    # In this approach, we learned NSRTs, so we just use the segmented
    # trajectories that NSRT learning returned to us.
    prompts = []
    assert len(segmented_trajs) == len(dataset.trajectories)
    for segment_traj, ll_traj in zip(segmented_trajs, dataset.trajectories):
        ...
        prompt = _create_prompt(init, goal, seg_options)
        prompts.append(prompt)
    return "\n\n".join(prompts) + "\n\n"


def _create_prompt(init: Set[GroundAtom], goal: Set[GroundAtom],
                   options: Sequence[_Option]) -> str:
    init_str = "\n  ".join(map(str, sorted(init)))
    goal_str = "\n  ".join(map(str, sorted(goal)))
    options_str = "\n  ".join(map(_option_to_str, options))
    prompt = f"""
(:init
{init_str}
)
...
class LLMOpenLoopApproach(NSRTMetacontrollerApproach):
    ...
    def _predict(self, state: State, atoms: Set[GroundAtom],
        ...
        action_seq = get_llm_based_plan(state, atoms, goal)
    ...
    def learn_from_offline_dataset(self, dataset: Dataset) -> None:
        ...
        self._prompt_prefix = data_to_prompt_prefix(dataset)
