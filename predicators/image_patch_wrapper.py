...
from predicators.utils import PyBulletState, BoundingBox
...
class VisImage:
    ...
    def save(self, filepath: str) -> None:
        """
        Args:
            filepath (str): absolute path (including file name) to save the image.
        """
...
class ImagePatch:
    def __init__(
        ...
        """
        Args:
            img: An image, which can be a PIL Image, a NumPy array, or a torch Tensor.
            left, lower, right, upper: The bounding box coordinates for cropping.
            parent_left, parent_lower: Offsets if this patch is part of a larger parent image.
        ...
        if left is None and right is None and upper is None and lower is None:
            ...
            # "upper" is the top, so the slice for the row dimension is h - upper : h - lower
            cropped = image_tensor[
                :,
                (h - _upper) : (h - _lower),
                _left:_right
            ]
    ...
    def label_all_objects(
        ...
        c, h, w = self.cropped_image.shape
        ...
        for obj, mask in obj_mask_dict.items():
            ...
            mask_dt = cv2.distanceTransform(mask_padded, cv2.DIST_L2, 0)
        ...
        self.cropped_image = th.tensor(vis_image.get_image()
                                       ).permute(2, 0, 1) / 255.0
