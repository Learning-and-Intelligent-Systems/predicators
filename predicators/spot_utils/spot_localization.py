...
from bosdyn.client.lease import Error as LeaseClient, LeaseKeepAlive
...
class SpotLocalizer:
    ...
    def __init__(self, robot: Robot, upload_path: Path, lease_client: LeaseClient,
    ...
    def localize(self, num_retries: int = 5, retry_wait_time: float = 1.0) -> math_helpers.SE3Pose:
        """Re-localize the robot. It's good practice to call this periodically
        to avoid drift issues. April tags need to be in view."""
        ...
        try:
            ...
            if str(transform) == "":
                raise LocalizationFailure("Transform invalid.")
        except (ResponseError, TimedOutError, LocalizationFailure):
            # Retry or fail.
            if num_retries <= 0:
                logging.warning("Localization failed permanently.")
                raise LocalizationFailure("Localization failed permanently.")
            logging.warning("Localization failed once, retrying.")
            time.sleep(retry_wait_time)
            return self.localize(num_retries=num_retries-1, retry_wait_time=retry_wait_time)
...
if __name__ == "__main__":
    ...
    args = utils.parse_args(env_required=False, seed_required=False, approach_required=False)
