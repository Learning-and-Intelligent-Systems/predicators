                    ...
                    if is_new and not obj.is_instance(
                        ...
                        for feature in obj.type.feature_names:
                            if feature == "x":
                                ...
                                features.append(state.get(obj, feature))
                        unknown.append(obj)
                        object_to_features[obj] = features
                        distance = x**2 + y**2
...
class MPDQNFunction(MapleQFunction):
    ...
    def __init__(self,
        ...
        super().__init__(seed, hid_sizes, max_train_iters, clip_gradients,
                         clip_value, learning_rate, weight_decay,
                         use_torch_gpu, train_print_every, n_iter_no_change,
                         discount, num_lookahead_samples,
                         replay_buffer_max_size,
                         replay_buffer_sample_with_replacement)

        # our most updated q network
        self.qnet = MapleQFunction(
            seed=CFG.seed,
        ...
        # target q network
        self.target_qnet = MapleQFunction(
            seed=CFG.seed,
            hid_sizes=CFG.mlp_regressor_hid_sizes,
            max_train_iters=CFG.mlp_regressor_max_itr,
            clip_gradients=CFG.mlp_regressor_clip_gradients,
    ...
    def _vectorize_option(self, option: _Option) -> Array:
        matches = [
            i for (i, n) in enumerate(self._options) if n == option.parent
        ]
    ...
    def train_q_function(self) -> None:
        ...
        if CFG.env == "doorknobs" and CFG.use_callplanner\
              and CFG.use_obj_centric:
            ...
            X_size = 142
        elif CFG.env == "grid_row_door" and CFG.use_callplanner \
            and CFG.use_obj_centric:
            X_size = 12
        ...
        for i, (state, _, option, next_state, reward,
                ...
                while len(next_option_vecs) < self._num_lookahead_samples:
                    ...
                    for next_option in \
                        ...
                        next_option_vecs.append(
                            self._vectorize_option(next_option))
                        actions_to_vectors[
                            next_option] = self._vectorize_option(next_option)
                for next_option in \
                        self._sample_applicable_options_from_state(
                            next_state):
                    x_hat = np.concatenate([
                        vectorized_next_state,
                        self._vectorize_option(next_option)
                    ])
            ...
            if best_next_value == 0.0:
                ...
                vectorized_next_action = self._vectorize_option(
                    next_best_action)
                x = np.concatenate(
                    [vectorized_next_state, vectorized_next_action])
                # as per double dqn, the q value is predicted by
                # #target_qnet and action is chosen by qnet
        ...
        if not self.target_qnet._disable_normalization: # pylint: disable=protected-access
            Xx, self.target_qnet._input_shift, \
                ...
                Xx)
            Yy, self.target_qnet._output_shift, \
                self.target_qnet._output_scale = _normalize_data(
                Yy)
        if not self._qfunc_init:
    ...
    def get_option(self,
        ...
        if self._rng.uniform() < epsilon:
            options = self._sample_applicable_options_from_state(
                state, num_samples_per_applicable_nsrt=1)
            # Note that this assumes that the output
            # of sampling is completely
            # random, including in the order of ground NSRTs.
            if self._use_epsilon_annealing and epsilon != 0:
                self.decay_epsilon()
        ...
        if CFG.polyak_true:
            for target_param, source_param in zip(
                ...
                target_param.data.copy_(
                    (1 - MPDQNFunction.tau) * target_param.data +
                    (MPDQNFunction.tau) * source_param.data)
        else:
            if CFG.env == "doorknobs":
                update_freq = 1500
            elif CFG.env == "grid_row_door":
