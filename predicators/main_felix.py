...
from predicators.structs import Dataset, InteractionRequest, InteractionResult, Metrics, Response, Task, Video, Action
...
assert os.environ.get("PYTHONHASHSEED") == "0", "Please add `export PYTHONHASHSEED=0` to your bash profile!"


def run_robo_kitchen_tests():
    """Run all the robo kitchen tests.
    ...
    try:
        ...
        from tests.robocasa_integration.test_robo_kitchen_actions import test_action_conversion, test_action_space, test_robo_kitchen_training_task_generation
        ...
        try:
            ...
            logging.error(f"✗ test_robo_kitchen_training_task_generation failed: {e}")
            task_gen_passed = False

        all_passed = action_conv_passed and action_space_passed and task_gen_passed
        ...
        logging.error("Make sure the test file exists at 'tests/robocasa_integration/test_robo_kitchen_actions.py'")
        return False
...
def main() -> None:
    ...
    # Check for test_robo_kitchen_actions flag directly
    if "--test_robo_kitchen_actions" in sys.argv:
        # Configure basic logging
        logging.basicConfig(level=logging.INFO, format="%(message)s")

        # Set necessary configuration values
        CFG.test_robo_kitchen_actions = True
        CFG.seed = 0
        CFG.num_test_tasks = 1
        CFG.robo_kitchen_randomize_init_state = True

        # Run tests
        run_robo_kitchen_tests()
        return
    ...
    logging.basicConfig(level=CFG.loglevel, format="%(message)s", handlers=handlers, force=True)
    ...
    preds = utils.replace_goals_with_agent_specific_goals(included_preds, excluded_preds, env) if CFG.approach != "oracle" else included_preds
    ...
    approach_train_tasks = [task.replace_goal_with_alt_goal() for task in stripped_train_tasks]
...
def _run_pipeline(env: BaseEnv, cogman: CogMan, train_tasks: List[Task], offline_dataset: Optional[Dataset] = None) -> None:
    ...
    if cogman.is_learning_based:
        ...
        offline_learning_metrics = {f"offline_learning_{k}": v for k, v in cogman.metrics.items()}
        ...
        for i in range(CFG.num_online_learning_cycles):
            ...
            interaction_results, query_cost = _generate_interaction_results(cogman, env, teacher, interaction_requests, i)
            num_online_transitions += sum(len(result.actions) for result in interaction_results)
...
def _generate_interaction_results(
    cogman: CogMan, env: BaseEnv, teacher: Optional[Teacher], requests: Sequence[InteractionRequest], cycle_num: Optional[int] = None
    ...
    for request in requests:
        if request.train_task_idx < CFG.max_initial_demos and not CFG.allow_interaction_in_demo_tasks:
            raise RuntimeError("Interaction requests cannot be on demo tasks " "if allow_interaction_in_demo_tasks is False.")
...
def _run_testing(env: BaseEnv, cogman: CogMan) -> Metrics:
    ...
    for test_task_idx, env_task in enumerate(test_tasks):
        ...
        try:
            ...
            logging.warning(f"Task {test_task_idx+1} / {len(test_tasks)}: " f"Approach failed to solve with error: {e}")
            ...
            if CFG.make_failure_videos and e.info.get("partial_refinements"):
                video = utils.create_video_from_partial_refinements(e.info["partial_refinements"], env, "test", test_task_idx, CFG.horizon)
        ...
        metrics[f"PER_TASK_task{test_task_idx}_nodes_created"] = cogman.metrics["total_num_nodes_created"] - curr_num_nodes_created
        metrics[f"PER_TASK_task{test_task_idx}_nodes_expanded"] = cogman.metrics["total_num_nodes_expanded"] - curr_num_nodes_expanded
        ...
        try:
            ...
            traj, solved, execution_metrics = run_episode_and_get_observations(cogman, env, "test", test_task_idx, max_num_steps=CFG.horizon, monitor=monitor)
            ...
            if CFG.refinement_data_include_execution_cost:
                total_low_level_action_cost += len(traj[1]) * CFG.refinement_data_low_level_execution_cost
            if CFG.save_eval_trajs:
                ...
                traj_data = {"task": env_task, "trajectory": traj, "pybullet_robot": CFG.pybullet_robot}
    ...
    metrics["avg_suc_time"] = total_suc_time / num_solved if num_solved > 0 else float("inf")
    metrics["avg_ref_cost"] = (total_low_level_action_cost + cogman.metrics["total_refinement_time"]) / num_solved if num_solved > 0 else float("inf")
    metrics["min_num_samples"] = cogman.metrics["min_num_samples"] if cogman.metrics["min_num_samples"] < float("inf") else 0
    metrics["max_num_samples"] = cogman.metrics["max_num_samples"]
    metrics["min_skeletons_optimized"] = cogman.metrics["min_num_skeletons_optimized"] if cogman.metrics["min_num_skeletons_optimized"] < float("inf") else 0
    ...
    for metric_name in ["num_samples", "num_skeletons_optimized", "num_nodes_expanded", "num_nodes_created", "num_nsrts", "num_preds", "plan_length", "num_failures_discovered"]:
        total = cogman.metrics[f"total_{metric_name}"]
        metrics[f"avg_{metric_name}"] = total / num_found_policy if num_found_policy > 0 else float("inf")
    return metrics
...
def _save_test_results(results: Metrics, online_learning_cycle: Optional[int]) -> None:
    ...
    outfile = f"{CFG.results_dir}/{utils.get_config_path_str()}__" f"{online_learning_cycle}.pkl"
