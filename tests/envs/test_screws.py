...
def test_screws():
    ...
    assert extreme_left_state.get(
        env.robot, "pose_x") == env.rz_x_lb + (env.gripper_width / 2.0)
    ...
    assert extreme_right_state.get(
        env.robot, "pose_x") == env.rz_x_ub - (env.gripper_width / 2.0)
    # Check that moving the robot beyond the bounds in the
    ...
    extreme_down_state = env.simulate(
        ...
        Action(
            np.array([0.0, -(env.rz_y_ub - env.rz_y_lb), 0.0],
                     dtype=np.float32)))
    assert extreme_down_state.get(env.robot, "pose_y") == env.rz_y_lb
    extreme_up_state = env.simulate(
        task.init,
        Action(
            np.array([0.0, (env.rz_y_ub - env.rz_y_lb), 0.0],
                     dtype=np.float32)))
    assert extreme_up_state.get(env.robot, "pose_y") == env.rz_y_ub
    ...
    move_to_screw_action = env.MoveToScrew_policy(task.init, {},
                                                  [env.robot, goal_screw],
                                                  np.zeros(1))
