...
def test_nsrt_learning_specific_nsrts():
    ...
    task1 = Task(init=state1,
                 goal=[
                     GroundAtom(OnTop, [hardback1, shelf]),
                     GroundAtom(OnTop, [hardback2, shelf])
                 ])
    task2 = Task(init=state1, goal=[GroundAtom(OnTop, [book1, shelf])])
    dataset = [
        LowLevelTrajectory([
        ...
        LowLevelTrajectory(
            ...
            [moveto_book1, pick_book1, movetoshelf1, place_book1])
    ]
    ...
    reverse_order_nsrts, _, _ = learn_nsrts_from_data(dataset, [task2, task1],
                                                      ...
                                                      action_space,
                                                      sampler_learner="random")
    assert len(nsrts) == len(reverse_order_nsrts)
