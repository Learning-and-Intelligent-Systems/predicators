...
from predicators.src.structs import Type, Predicate, STRIPSOperator, Segment, \
    PartialNSRTAndDatastore, Action, LowLevelTrajectory, Task, State
...
def test_backchaining_strips_learner():
    ...
    learner = MockBackchainingSTRIPSLearner(
        [traj1, traj2], [task1, task2], {Asleep}, [[segment1], [segment2]])
    pnads = learner.learn()
    ...
    learner = MockBackchainingSTRIPSLearner(
        [traj3, traj4], [task3], {Asleep, Sad}, [[segment3], [segment4]])
    pnads = learner.learn()
    assert len(pnads) == 1
...
def test_find_unification_and_try_specializing_pnad():
    ...
    learner = MockBackchainingSTRIPSLearner(
        [traj], [task], {Asleep, Happy}, [[segment]])
    ...
    ground_op = learner.find_unification(
        set(), pnad, Segment(traj, set(), set(), Move), False)
    assert ground_op is None
