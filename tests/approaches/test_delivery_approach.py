...
from predicators.envs.pddl_env import _PDDLEnvState, ProceduralTasksDeliveryPDDLEnv
...
def test_delivery_approach():
    ...
    approach = create_approach(CFG.approach, env.predicates, get_gt_options(env.get_name()), env.types, env.action_space, [])
    ...
    for idx, task in enumerate(tasks):
        ...
        if idx == 0:
            ...
            starting_loc_ground_atom = next(filter(lambda atom: atom.predicate == at_pred, init_state.get_ground_atoms()))
            ...
            loc = next(filter(lambda loc: loc != starting_loc, task.init.get_objects(loc_type)))
        ...
        traj = utils.run_policy_with_simulator(policy, env.simulate, task.init, task.goal_holds, max_num_steps=CFG.horizon)
