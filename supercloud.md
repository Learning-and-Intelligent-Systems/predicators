...
echo "alias sl='squeue --format=\"%.18i %.9P %.40j %.8u %.8T %.10M %.6D %R\"'" >> ~/.bashrc
source ~/.bashrc
```
...
```
Running command: sbatch -p normal --time=99:00:00 --partition=xeon-p8 --nodes=1 --exclusive --job-name=cover_oracle__456 -o /tmp/%j_log.out run_cover_oracle__456.sh
Started job, see log with:
tail -n 10000 -F logs/cover_oracle__456.log
```

After experiments are running:
* To monitor experiments that are running, use `sl`.
* As indicated by the printouts, to see individual logs, you can use, for example, `tail -n 10000 -F logs/cover_oracle__456.log`.
