...
model_types = ['geometry+', 'specialized', 'generic', 'oracle']
total_tasks = '_500'

model_map = {'geometry+': 'Mixture', 'specialized': 'Specialized', 'generic': 'Generic', 'oracle': 'Uniform'}
...
for n_tasks  in num_tasks_per_cycle_list:
    ...
    for model in model_types:
        for approach in (lifelong_approaches if model != 'oracle' else [None]):
            ...
            for seed in range(num_seeds):
                for n_iter in range(num_iters):
                    if model in ['specialized', 'generic']:
                        ...
                        fname = f'results_lifelong/{model}_{n_tasks}{total_tasks}/planar_behavior__lifelong_sampler_learning__{seed}________{n_iter}.pkl'
                    else:
    ...
    for model in model_types:
        for approach in (lifelong_approaches if model != 'oracle' else [None]):
            ...
            if model != 'oracle':
                ...
                plt.figure(0)
                plt.plot(successes[model,approach].mean(axis=0), label=f'{model_legend}', linewidth=4, linestyle=':', color='black')
                plt.figure(1)
                plt.plot(samples[model,approach].mean(axis=0).cumsum(axis=0), successes[model,approach].mean(axis=0), label=f'{model_legend}', linewidth=4, linestyle=':', color='black')
                plt.figure(2)
                plt.plot(samples[model,approach].mean(axis=0).cumsum(axis=0), successes[model,approach].mean(axis=0).cumsum(axis=0), label=f'{model_legend}', linewidth=4, linestyle=':', color='black')
                # plt.errorbar(samples[model, approach].mean(axis=0).cumsum(axis=0), successes[model, approach].mean(axis=0).cumsum(axis=0), yerr=successes[model, approach].cumsum(axis=1).std(axis=0) / np.sqrt(num_seeds))
                plt.figure(3)
                div = np.full(samples[model, approach].shape[1], n_tasks)
                div[0] = burnin
                plt.plot(samples[model,approach].mean(axis=0).cumsum(axis=0), samples[model,approach].mean(axis=0) / div, label=f'{model_legend}', linewidth=4, linestyle=':', color='black')
                plt.figure(4, figsize=(18,6))
                plt.plot(np.array([0]), np.array([0]), label=f'{model_legend}', linewidth=4, linestyle=':', color='black')
    ...
    plt.tight_layout()
    plt.savefig(f'results_lifelong/avg_solved_per_iter_{n_tasks}_rebuttal.pdf')
    plt.close()
    plt.figure(1)
    plt.legend()
    ...
    plt.ylabel('# solved tasks per iter')
    plt.tight_layout()
    plt.savefig(f'results_lifelong/avg_solved_per_sample_{n_tasks}_rebuttal.pdf')
    plt.close()
    plt.figure(2)
    ...
    plt.tight_layout()
    plt.savefig(f'results_lifelong/total_solved_{n_tasks}_rebuttal.pdf')
    plt.close()
    plt.figure(3)
    plt.legend()
    plt.xlabel('# compute units')
    plt.ylabel('# samples per attempted task')
    plt.savefig(f'results_lifelong/avg_samples_{n_tasks}_rebuttal.pdf')
    plt.tight_layout()
    plt.close()
    plt.figure(4, figsize=(18,6))
    plt.legend(ncol=2)
    plt.savefig('results_lifelong/legend_lifelong_rebuttal.pdf')
    plt.close()
