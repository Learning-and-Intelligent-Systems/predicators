...
def run_low_level_search(
    ...
    """
    print("running low level search")
    print(skeleton[-1].name)
...
def run_backtracking_with_previous_states(
    ...
    assert not CFG.sesame_check_expected_atoms or (len(skeleton) == len(atoms_sequence) and goal.issubset(atoms_sequence[-1]))
...
def _backtrack(
    ...
    for iter in range(max_tries[current_depth]):
        ...
        print(f"_backtrack depth:{len(next_states) - 1} iter:{iter}")
