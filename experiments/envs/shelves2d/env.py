...
__all__ = ['Shelves2DEnv', 'SimulatorState']


@dataclass
class SimulatorState:
    leftmost_shelf: Object
...
class Shelves2DEnv(BaseEnv):
    ...
    range_world_y: ClassVar[Tuple[(float, float)]] = (-80, 80)
    ...
    min_box_width_clearance: ClassVar[float] = 1
    min_box_height_clearance: ClassVar[float] = 3

    cover_thickness: ClassVar[float] = 3
    ...
    def _generate_test_tasks(self) -> List[EnvironmentTask]:
        if self._test_tasks is None:
            self._test_tasks = self._get_tasks(
                ...
                range_shelves=(CFG.shelves2d_test_num_boxes,
                               CFG.shelves2d_test_num_boxes),
                rng=self._test_rng
            )
        # self._generate_tasks()
    ...
    def _generate_tasks(self):
        if self._tasks is None:
            ...
            self._tasks = [real_task for _, real_task in zip(
                range(CFG.num_train_tasks), itertools.cycle(real_tasks))]
    ...
    def action_space(self) -> gym.spaces.Box:
        """The action space is as follows:
         0, 1 - grabbed object coords
         2, 3 - destination object coords
         4, 5 - relative pose
        """
        lower = np.array([
    ...
    def simulate(self, state: State, action: Action) -> State:
        ...
        grasp_x, grasp_y, put_x, put_y, offset_x, offset_y = action.arr
        aux_data = cast(SimulatorState, state.simulator_state)
        next_state = state.copy()

        # Extract objects
        movables = state.get_objects(
            self._box_type) + state.get_objects(self._cover_type)
        shelves = state.get_objects(self._shelf_type)

        # Check which object is grasped and put relative to
        grasped_obj = self._get_touched_obj(state, grasp_x, grasp_y)
        put_obj = self._get_touched_obj(state, put_x, put_y)
        if grasped_obj is None:
            logging.info("NO GRASPED OBJECT")
            return next_state
        if put_obj is None:
            logging.info("NO PUT OBJECT")
            return next_state
        if grasped_obj.is_instance(self._shelf_type):
            logging.info("GRASPED OBJECT IS SHELF")
            return next_state

        # Move the grasped object
        if grasped_obj.is_instance(self._box_type) and not self._is_cover_at_start(state, aux_data.box2cover(grasped_obj)):
            logging.info("COVER NOT AT START")
            return next_state

        grasped_obj_x, grasped_obj_y, _, _ = self.get_shape_data(
            state, grasped_obj)
        put_obj_x, put_obj_y, _, _ = self.get_shape_data(state, put_obj)

        grasped_poly = translate(
            self._get_obj_polygon(state, grasped_obj),
            put_obj_x - grasped_obj_x + offset_x,
            put_obj_y - grasped_obj_y + offset_y
        )

        # Check if within bounds of the world
        if not self._world_poly.contains(grasped_poly):
            logging.info("OOB")
            return next_state

        # Check if intersects movable objects
        if MultiPolygon(
            self._get_obj_polygon(state, obj) for obj in state.get_objects(self._cover_type) + state.get_objects(self._box_type)
            if obj is not grasped_obj
        ).intersects(grasped_poly):
            logging.info("COLLISION WITH MOVABLE OBJECT")
            return next_state

        # Check if intersects shelf sides
        if MultiLineString([side for shelf in state.get_objects(self._shelf_type) for side in self._get_shelf_sides(state, shelf)]).intersects(grasped_poly):
            logging.info("COLLISION WITH SHELF")
            return next_state

        next_state.set(grasped_obj, 'pose_x', put_obj_x + offset_x)
        next_state.set(grasped_obj, 'pose_y', put_obj_y + offset_y)
        return next_state

    def _get_tasks(self, num_tasks: int, range_shelves, rng: np.random.Generator) -> List[EnvironmentTask]:
        return [self._get_task(range_shelves, rng) for _ in range(num_tasks)]

    def _get_task(self, range_shelves: Tuple[int, int], rng: np.random.Generator) -> EnvironmentTask:
        num_shelves = rng.integers(range_shelves[0], range_shelves[1] + 1)

        # Create objects belonging to the subtasks
        boxes = [Object(f'box_{idx}', self._box_type)
                 for idx in range(num_shelves)]
        shelves = [Object(f'shelf_{idx}', self._shelf_type)
                   for idx in range(num_shelves)]
        bundle = Object(f'bundle', self._bundle_type)
        cover = Object(f'cover', self._cover_type)

        # Create the state and exclusion polygon
        state = State({}, SimulatorState(shelves[0]))
        ...
        goal = {cover_predicate([cover, bundle])} | {self._In(
            [box, shelf]) for box, shelf in zip(boxes, shelves)}
        ...
        for box in boxes:
            for _ in range(self.num_tries):
                box_w = rng.uniform(*self.range_box_width)
                box_h = rng.uniform(*self.range_box_height)
                box_x = rng.uniform(
                    self.range_world_x[0], self.range_world_x[1] - box_w)
                box_y = rng.uniform(
                    self.range_world_y[0], self.range_world_y[1] - box_h)
                box_poly = BoxWH(box_x, box_y, box_w, box_h)
                if self._world_poly.contains(box_poly) and not objs_multipoly.intersects(box_poly):
                    break
            else:
                raise ValueError(
                    'Could not generate a task with given settings')

            state.data[box] = np.array(
                [box_x, box_y, box_w, box_h], dtype=np.float32)
        ...
        # Generate the bundle
        for _ in range(self.num_tries):
            shelf_widths = rng.uniform(
                np.maximum(box_widths + self.min_box_width_clearance,
                           self.range_shelf_width[0]),
                self.range_shelf_width[1]
            )
            shelf_seps = rng.uniform(*self.range_shelf_sep, num_shelves - 1)

            bundle_width = shelf_widths.sum() + shelf_seps.sum()
            bundle_height = rng.uniform(
                self.range_shelf_height[0],
                min(self.range_shelf_height[1],
                    min_box_height - self.min_box_height_clearance)
            )

            bundle_x = rng.uniform(
                self.range_world_x[0], self.range_world_x[1] - bundle_width)
            bundle_y = rng.uniform(
                self.range_world_y[0], self.range_world_y[1] - bundle_height)

            bundle_margin_x = self.cover_sideways_tolerance + self.bundle_margin
            bundle_margin_y = max(
                max_box_height, self.cover_max_distance + self.cover_thickness) + self.bundle_margin
        ...
        state.data[bundle] = np.array(
            [bundle_x, bundle_y, bundle_width, bundle_height], dtype=np.float32)
        ...
        for shelf, shelf_width, shelf_sep in zip(shelves, shelf_widths, list(shelf_seps) + [0]):
            ...
            shelf_poly = BoxWH(shelves_x, shelves_y,
                               shelf_width, bundle_height)
        ...
        for _ in range(self.num_tries):
            ...
            cover_x = rng.uniform(
                self.range_world_x[0], self.range_world_x[1] - cover_w)
            cover_y = rng.uniform(
                self.range_world_y[0], self.range_world_y[1] - cover_h)
    ...
    def _GoesInto_holds(self, state: State, objects: Sequence[Object]) -> bool:
        (box, shelf) = objects
        box2shelf = cast(SimulatorState, state.simulator_state).box2shelf
        return box2shelf[box] == shelf

    def _Bundles_holds(self, state: State, objects: Sequence[Object]) -> bool:
        (bundle, shelf) = objects
        shelf2bundle = cast(SimulatorState, state.simulator_state).shelf2bundle
    ...
    def _CoverFor_holds(self, state: State, objects: Sequence[Object]) -> bool:
        (cover, bundle) = objects
        bundle2cover = cast(SimulatorState, state.simulator_state).bundle2cover
    ...
    @classmethod
    def _get_touched_obj(cls, state: State, x: float, y: float) -> Optional[Object]:
        touch_point = Point(x, y)
        movables = state.get_objects(
            cls._box_type) + state.get_objects(cls._cover_type)
        obj_polys = [
            (obj, cls._get_obj_polygon(state, obj)) for obj in state.get_objects(cls._box_type) +
            state.get_objects(cls._cover_type) +
            state.get_objects(cls._shelf_type)
        ]
        return next(filter(lambda p: p[1].contains(touch_point), obj_polys), (None, None))[0]
