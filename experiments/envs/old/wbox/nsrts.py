...
__all__ = ['WBoxGroundTruthNSRTFactory']

class WBoxGroundTruthNSRTFactory(GroundTruthNSRTFactory):
    # Settings
    placement_margin = 0.1
    grasp_margin = 0.1
    move_search_margin = 3
    next_to_margin = 0.4
    ...
    def get_nsrts(
        ...
        def Grab_sampler(
            ...
            _, _, block = objects
            block_shape = WBox._get_shape(state, block).buffer(-cls.grasp_margin, join_style="mitre")
        ...
        def MoveToBlock_sampler(
            ...
            _, _, block = objects
            block_shape = WBox._get_shape(state, block)
            ...
            while True:
                ...
                if Point(x, y).distance(WBox._get_shape(state, block)) <= cls.next_to_margin:
                    break

            return np.array([0, 1.0, 0, x, y])
        ...
        def MoveToContainer_sampler(
            ...
            desired_x, desired_y = WBox._get_desired_pos(state, block)
            desired_block_shape = WBox._get_shape(state, block, desired_x, desired_y)
            ...
            return np.array([0.0, 1.0, 0.0, x, y])
        ...
        def Place_sampler(
            ...
            while True:
                sampling_margin = WBox.sub_cell_margin - cls.placement_margin
                x, y = rng.uniform(
                    (desired_x - sampling_margin, desired_y - sampling_margin),
                    (desired_x + sampling_margin, desired_y + sampling_margin)
                )
                if Point(0, 0).distance(WBox._get_shape(state, block, x, y)) <= cls.next_to_margin:
                    break
            return np.array([0, 0, 1, x, y])
