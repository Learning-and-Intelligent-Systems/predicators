...
for ENV in ${ALL_ENVS[@]}; do
    ...
    for DEMOS in ${NUM_DEMOS[@]}; do
        if [ "$RUN_LOAD_EXPERIMENTS" = true ]; then
            ...
            python $FILE $COMMON_ARGS --experiment_id ${ENV}_pnadsearch_${DEMOS}demo --approach nsrt_learning --strips_learner pnad_search --num_train_tasks ${DEMOS} --pnad_search_timeout 50000
