...
for SEED in $(seq $START_SEED $((NUM_SEEDS+START_SEED-1))); do
    ...
    python $FILE --experiment_id rnt_sing_opt_oracle --env repeated_nextto_single_option --approach oracle --seed $SEED --num_train_tasks 0
    python $FILE --experiment_id rnt_sing_opt_no_sidepreds --env repeated_nextto --approach nsrt_learning --side_predicate_learner no_learning --seed $SEED --num_train_tasks 50
    python $FILE --experiment_id rnt_sing_opt_backchain --env repeated_nextto_single_option --approach nsrt_learning --side_predicate_learner backchaining --seed $SEED --num_train_tasks 50
done
