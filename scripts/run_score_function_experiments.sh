...
for SEED in $(seq $START_SEED $((NUM_SEEDS+START_SEED-1))); do
    ...
    for THRESH in $(seq 0.1 0.1 0.3); do
        python $FILE --env cover --approach interactive_learning --seed $SEED --excluded_predicates Covers,Holding --experiment_id excludeall_entropy_$THRESH --interactive_query_policy threshold --interactive_score_threshold $THRESH --interactive_score_function entropy --num_online_learning_cycles $CYCLES --max_num_steps_interaction_request $MAX_STEPS #--load_approach --make_interaction_videos
    done
    # ours, BALD
