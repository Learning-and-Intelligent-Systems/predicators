...
ROW_GROUPS = [
    ("Screws", pd_create_equal_selector("ENV", "screws")),
    ("Repeated NextTo", pd_create_equal_selector("ENV", "repeated_nextto_single_option")),
    ("RNT Painting", pd_create_equal_selector("ENV", "repeated_nextto_painting")),
    ("Painting", pd_create_equal_selector("ENV", "painting")),
    ("Satellites", pd_create_equal_selector("ENV", "satellites")),
    ("Satellites Simples", pd_create_equal_selector("ENV", "satellites_simple")),
]

# See ROW_GROUPS comment.
OUTER_HEADER_GROUPS = [
    ("Ours", lambda df: df["EXPERIMENT_ID"].apply(lambda v: "_backchaining_50" in v)),
    ("Cluster and Intersect", lambda df: df["EXPERIMENT_ID"].apply(
        lambda v: "_cluster_and_intersect_50" in v)),
    ("LOFT",
     lambda df: df["EXPERIMENT_ID"].apply(lambda v: "_cluster_and_search_50" in v)),
    ("Pred. Error", lambda df: df["EXPERIMENT_ID"].apply(
        lambda v: "_pred_error_50" in v)),
    # ("GNN Shooting", lambda df: df["EXPERIMENT_ID"].apply(
    #     lambda v: "_gnn_shooting_50" in v)),
    # ("GNN Model-Free", lambda df: df["EXPERIMENT_ID"].apply(
    #     lambda v: "_gnn_modelfree_50" in v)),
