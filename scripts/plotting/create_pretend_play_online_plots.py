...
PLOT_GROUPS = {
    "Blocks": [
        ("Blocks", "blue",
         lambda df: df["EXPERIMENT_ID"].apply(lambda v: "pretend_play_blocks" in v)),
    ],
}
...
# Plot type.
PLOT_TYPE = "errorbars"  # single_lines or seed_lines
...
def _create_errorbar_plot(ax: plt.Axes, df: pd.DataFrame,
                          plot_group: Sequence[Tuple[str, str, Callable]],
                          x_key: str, y_key: str) -> None:
    for label, color, selector in plot_group:
        entry_df = get_df_for_entry(x_key, df, selector)
        ...
        for y_key, y_label in Y_KEY_AND_LABEL:
            for plot_title, d in PLOT_GROUPS.items():
                ...
                if PLOT_TYPE == "seed_lines":
                    ...
                    _create_errorbar_plot(ax, df, d, x_key, y_key)
                else:
