...
Example command line:
    ...
    python src/main.py --approach analysis --seed 0 \
...
class AnalysisApproach(NSRTMetacontrollerApproach):
    ...
    def _create_prompt(self, init: Set[GroundAtom], goal: Set[GroundAtom],
        ...
        for i in init:
            ...
            if sub in existing:
                ...
                init_arr.append(existing_new[existing.index(sub)] +
                                i[i.index('('):])

            else:
                existing.append(sub)
                sub_random = ''.join(self._generate_guess(sub))
                existing_new.append(sub_random)
        ...
        for i in goal:
            ...
            sub = i[0:i.index('(')]
            if sub in existing:
                goal_arr.append(existing_new[existing.index(sub)] +
                                i[i.index('('):])
            else:
                existing.append(sub)
                sub_random = ''.join(self._generate_guess(sub))
