...
Example command line:
    ...
    python src/main.py --approach analysis_two --seed 0 \
...
class AnalysisTwoApproach(NSRTMetacontrollerApproach):
                ...
                if j in option_str:
                    ...
                    options_str_list[options_str_list.index(
                        option_str)] = option_str.replace(
                            sub, original_strings[random_strings.index(j)])
                    break
        for option_str in options_str_list:
    ...
    def _create_prompt(self, init: Set[GroundAtom], goal: Set[GroundAtom],
        ...
        for option in options:
            sub = option.name
            if sub in original_strings:
                option.name = random_strings[original_strings.index(sub)]
            else:
                original_strings.append(sub)
                sub_random = ''.join(self._generate_guess(sub))
                random_strings.append(sub_random)
