...
class DaggerLearningApproach(NSRTLearningApproach):
    ...
    def learn_from_interaction_results(self, results: Sequence[InteractionResult], env: BaseEnv) -> None:
        ...
        for result in results:  # one result per training task
            ...
            for res in responses:
                ...
                for s in teacher_traj.states:
                    ...
                    video.extend(env.render_state(s, dummy_task))
                video_prefix = utils.get_config_path_str()
                n = np.random.randint(0, 1000)
                outfile = f"{video_prefix}_teacher_demos_{n}.mp4"
