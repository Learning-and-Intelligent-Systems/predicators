...
class STRIPSOperator:
    ...
    def effect_to_side_predicate(self, effect: LiftedAtom,
        ...
        """Return a new STRIPS operator resulting from turning the
        given effect (either add or delete) into a side predicate.
        """
        ...
        remaining_params = {p for atom in self.preconditions |
        ...
        return STRIPSOperator(
            self.name, new_params, self.preconditions,
            new_add_effects, new_delete_effects,
...
class Segment:
    ...
    def get_option_spec(self) -> OptionSpec:
        """Get the option spec for this segment.
        """
        if self.has_option():
            segment_option = self.get_option()
            segment_param_option = segment_option.parent
            segment_option_objs = tuple(segment_option.objects)
        else:
            segment_param_option = DummyOption.parent
            segment_option_objs = tuple()
        return (segment_param_option, segment_option_objs)
