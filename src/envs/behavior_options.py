...
def detect_robot_collision(robot: "BaseRobot") -> bool:
    ...
    return (detect_collision(robot.parts["body"].body_id, object_in_hand)
...
def navigate_to_param_sampler(state: State, goal: Set[GroundAtom],
    ...
    num_samples_tried = 0
    while (check_nav_end_pose(env.igibson_behavior_env, obj_to_sample_near, sampler_output) is None):
    ...
    assert check_nav_end_pose(env.igibson_behavior_env, obj_to_sample_near, sampler_output) is not None
...
def check_nav_end_pose(
    ...
    """Check that the robot can reach pos_offset from the
    obj without (1) being in collision with anything, or (2) being blocked
    from obj by some other solid object. If this is true, return the
    ((x,y,z),(roll, pitch, yaw)), else return None"""
    valid_position = None
    state = p.saveState()
