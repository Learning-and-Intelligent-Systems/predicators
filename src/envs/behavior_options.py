...
def get_delta_low_level_base_action(
    ...
    old_orn_quat = p.getQuaternionFromEuler(
        np.array(
            [original_orientation[0], original_orientation[1], old_xytheta[2]]
        )
    )
    new_pos = np.array([new_xytheta[0], new_xytheta[1], robot_z])
    new_orn_quat = p.getQuaternionFromEuler(
        np.array(
            [original_orientation[0], original_orientation[1], new_xytheta[2]]
        )
    )
    ...
    delta_pos, delta_orn_quat = p.multiplyTransforms(
        inverted_old_pos, inverted_old_orn_quat, new_pos, new_orn_quat
    )
...
def navigate_to_obj_pos(env, obj, pos_offset, rng=np.random.default_rng(23)):
    ...
    def sample_fn(env, rng):
        ...
        theta = (
            rng.random() * (CIRCULAR_LIMITS[1] - CIRCULAR_LIMITS[0])
        ) + CIRCULAR_LIMITS[0]
        return (x, y, theta)

    if isinstance(
        obj, URDFObject
    ):  # must be a URDFObject so we can get its position!
...
def grasp_obj_at_pos(
    ...
    if env.obj_in_hand is None:
            ...
            if (
                ...
                if (
                    ...
                    if plan is not None:
                        ...
                        print(
                            f"PRIMITIVE: grasp {obj.name} fail, failed to find plan to continuous params {grasp_offset_and_z_rot}"
                        )
                        return None
                ...
                print(
                    "PRIMITIVE: grasp {} fail, too big or fixed".format(
                        obj.name
                    )
                )
                return None
        ...
        print(
            "PRIMITIVE: grasp {} fail, agent already has an object in hand!".format(
                obj.name
            )
        )
        return None
...
def place_ontop_obj_pos(
    ...
    if obj_in_hand is not None and obj_in_hand != obj:
        print(
            "PRIMITIVE:attempt to place {} ontop {}".format(
        ...
        if isinstance(obj, URDFObject):
            if (
                ...
                if not option_model:
                    ...
                    print(
                        "PRIMITIVE: place {} ontop {} success".format(
                            obj_in_hand.name, obj.name
                        )
                    )
                ...
                print(
                    "PRIMITIVE: place {} ontop {} fail, too far".format(
                        obj_in_hand.name, obj.name
                    )
                )
                return None
