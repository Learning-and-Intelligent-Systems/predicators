...
def make_behavior_option(  # type: ignore
    ...
    def _model(
        ...
        assert (
            "controller_model" in memory and memory["controller_model"] is not None
        )  # must call initiable() first, and it must return True
        next_state, _ = memory["controller_model"](state, env)
        return next_state
    ...
    def _initiable(
        ...
        if memory.get("controller") is None:
            ...
            if state.simulator_state is not None:
                env._env.task.reset_scene(state.simulator_state)
            controller = controller_fn(env, igo[0], params, option_model=False, rng=rng)
            memory["controller"] = controller
            controller_model = controller_fn(env, igo[0], params, option_model=True, rng=rng)
            memory["controller_model"] = controller_model
...
class BehaviorEnv(BaseEnv):
    ...
    def _reachable_classifier(self, state: State, objs: Sequence[Object]) -> bool:
    ...
    def _reachable_nothing_classifier(self, state: State, objs: Sequence[Object]) -> bool:
        # Check allclose() here for uniformity with _create_classifier_from_bddl
        assert state.allclose(self._current_ig_state_to_state())
        assert len(objs) == 1
        for obj in state:
