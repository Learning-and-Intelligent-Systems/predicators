...
def learn_nsrts_from_data(
    ...
    #     if i == max_num_orderings - 1:
    #         rnd_seed = min_rnd_seed
    #     else:
    rnd_seed = 810552 #int(random.random() * 1000000)
    if CFG.sort_data != "default":
        ...
        elif CFG.sort_data == "all_first":
            trajectories = [trajectories[-1]] + trajectories[:-1]
            random.seed(rnd_seed)
            ground_atom_dataset = [ground_atom_dataset[-1]] + ground_atom_dataset[:-1]
        else:
            raise NotImplementedError(f"Not a vaild sorting heuristic {CFG.sort_data}")

        if CFG.sort_data != "all_first":
    ...
    # if CFG.sort_data == "all_first" and i == len(trajectories):
    #     break
    
    #import ipdb; ipdb.set_trace()

    if len(pnads) > 8:
        print("Broke - PNADs of size:")
        print(len(pnads))
        quit()

    CFG.min_perc_data_for_nsrt = 5
    pnads = learn_strips_operators(
            trajectories,
            train_tasks,
            predicates,
