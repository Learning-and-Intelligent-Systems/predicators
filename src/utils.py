...
def create_pddl_domain(operators: Collection[Union[STRIPSOperator, NSRT]],
                       ...
                       domain_name: str = "autogenerated") -> str:
    ...
    preds_str = "\n    ".join(map(_predicate_to_pddl_str, preds_lst))
    ops_strs = "\n\n  ".join(map(_operator_to_pddl_str, ops_lst))
...
def create_pddl_problem(objects: Collection[Object],
                        init_atoms: Collection[GroundAtom],
                        goal: Collection[GroundAtom],
                        domain_name: str = "autogenerated",
                        problem_name: str = "autogenerated") -> str:
    ...
    init_str = "\n    ".join(map(_atom_to_pddl_str, init_atoms_lst))
    goal_str = "\n    ".join(map(_atom_to_pddl_str, goal_lst))
...
def _predicate_to_pddl_str(predicate: Predicate) -> str:
    """Helper for create_pddl_domain.
    """
    if predicate.arity == 0:
        return f"({predicate.name})"
    vars_str = " ".join(f"?x{i} - {t.name}"
                        for i, t in enumerate(predicate.types))
    return f"({predicate.name} {vars_str})"


def _operator_to_pddl_str(operator: Union[STRIPSOperator, NSRT]) -> str:
    """Helper for create_pddl_domain.
    """
...
def _atom_to_pddl_str(atom: _Atom) -> str:
    """Helper for create_pddl_domain and create_pddl_problem.
    """
    if not atom.entities:
        return f"({atom.predicate.name})"
    entities_str = " ".join(e.name for e in atom.entities)
    return f"({atom.predicate.name} {entities_str})"
